#!/usr/bin/env bash
# Emergency Contabo recovery when the public site returns 503 and PM2 is broken.
# Typical symptoms:
#   - curl 127.0.0.1:3000 → connection refused
#   - https://www.arabya.org → 503
#   - pm2 restart arabya-web → "Process 0 not found"
#   - contabo-deploy.sh → "Cannot fast-forward to multiple branches"
#
# Usage (as root on Contabo):
...
echo "==> 1) Sync git to origin/${BRANCH} (avoid broken git pull)"
...
# Prefer a full healthy deploy when possible.
if [[ "${CONTABO_RECOVER_FULL:-1}" == "1" ]]; then
  echo "==> 3) Full deploy (npm ci + build + start)"
  bash "$APP_DIR/scripts/contabo-deploy.sh"
  exit $?
fi

# Fast path: restore last good build if present and node_modules looks usable
...
if contabo_tree_ready; then
  ...
  echo "ERROR: tree not ready for next start — run full deploy:"
