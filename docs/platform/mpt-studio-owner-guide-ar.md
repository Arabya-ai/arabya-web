...
ضع **فيديوهات MP4 حقيقية** (أو صورًا حقيقية وليست أرقام الاختبار) في `services/money-printer-turbo/storage/local_videos/`. ملفات `1.png` … `9.png` صور أرقام تجريبية؛ استخدامها يجعل الفيديو شرائح ثابتة برقم كبير. الواجهة لن تختارها بعد الآن.

للمقاطع المتحركة المرتبطة بالموضوع: مصدر **Pexels**. ضع `PEXELS_API_KEY` في `.env.production.local` (نفس مفتاح استوديو الآيات إن وُجد) — سكربت النشر ينسخه إلى المحرك.

صوت Edge TTS لا يحتاج مفتاحًا.

## مفاتيح مطلوبة داخل `config.toml` لمسار «موضوع → سكربت + مخزون»

- مفتاح نموذج لغوي (OpenAI / Gemini / غيرها حسب اختيارك في ملف الإعداد)
- مفتاح Pexels أو Pixabay لمقاطع المخزون. إن كان `PEXELS_API_KEY` موجودًا أصلًا لاستوديو الآيات، سكربت النشر يمرّره للمحرك تلقائيًا.
