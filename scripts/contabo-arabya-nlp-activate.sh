...
if grep -q '^ARABYA_NLP_PORT=' "$ENV_FILE" 2>/dev/null; then
  ...
  echo "ARABYA_NLP_PORT=${PORT}" >> "$ENV_FILE"
fi
...
grep -E '^(ARABYA_NLP_HOST|ARABYA_NLP_PORT|ARABYA_NLP_DEVOPS_AUTO_EXECUTE)=' "$ENV_FILE" || true
...
echo "==> Step 4/4 — health check (JSON required; ServerAvatar HTML = FAIL)"
sleep 2
# Diagnose who owns :8092 before curling
ss -tlnp 2>/dev/null | grep -E ":${PORT}\\b" || netstat -tlnp 2>/dev/null | grep -E ":${PORT}\\b" || true
...
# Reject ServerAvatar / OpenLiteSpeed HTML hijack
if echo "$HEALTH_JSON" | grep -qiE '<html|serveravatar|openlitespeed|welcome to'; then
  echo "ERROR: got HTML instead of JSON — port ${PORT} is hijacked or FastAPI not bound."
  echo "Fix: bash scripts/contabo-arabya-nlp-firewall.sh && bash scripts/contabo-arabya-nlp.sh"
  echo "Also add ServerAvatar inbound TCP ${PORT} — docs/platform/arabya-nlp-port-8092-ar.md"
  pm2 logs arabya-nlp --lines 40 --nostream || true
  exit 1
fi
